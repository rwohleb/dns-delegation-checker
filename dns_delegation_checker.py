...
class DNSDelegationChecker:
    ...
    def _fetch_root_servers(self) -> List[str]:
        ...
        for resolver_ip in public_resolvers:
            try:
                ...
                if root_servers:
                    print(
                        f"Successfully fetched {len(root_servers)} root servers dynamically using {resolver_ip}"
                    )
                    return root_servers
        ...
        print(
            "Warning: Could not fetch root servers dynamically from any public resolver"
        )
    ...
    def query_authoritative(
        self, domain: str, record_type: str, nameservers: List[str]
    ...
    def get_nameservers_for_zone(
        self, zone_name: str, parent_nameservers: List[str]
            ...
            for rrset in answer_rrsets + authority_rrsets:
                if hasattr(rrset, "rdtype") and rrset.rdtype == dns.rdatatype.NS:
                    ...
                    if rrset_name == zone_name:
                        ...
                        rrset_name == zone_name.split(".")[-1]
                    ):  # Parent zone (e.g., 'com' for 'gravyty.com')
                        ...
                        if parent_ns:
                            child_response = self.query_authoritative(
                                zone_name, "NS", parent_ns
                            )
                            if child_response:
                                child_answer_rrsets = getattr(
                                    child_response, "answer", []
                                )
                                child_authority_rrsets = getattr(
                                    child_response, "authority", []
                                )

                                for child_rrset in (
                                    child_answer_rrsets + child_authority_rrsets
                                ):
                                    if (
                                        hasattr(child_rrset, "rdtype")
                                        and child_rrset.rdtype == dns.rdatatype.NS
                                    ):
                                        if (
                                            str(child_rrset.name).rstrip(".")
                                            == zone_name
                                        ):
    ...
    def check_problematic_records(
        ...
        for record_type in problematic_types:
            ...
            if response:
                ...
                for rrset in answer_rrsets + authority_rrsets:
                    if hasattr(rrset, "rdtype") and rrset.rdtype == getattr(
                        dns.rdatatype, record_type
                    ):
                        for rdata in rrset:
    ...
    def find_delegation_chain_nameservers(
        ...
        for i in range(len(domain_parts) - 1, 0, -1):
            ...
            parent_zone_ns, _ = self.get_nameservers_for_zone(
                parent_zone, current_nameservers
            )
    ...
    def check_delegation_chain(self, target_domain: str) -> List[DelegationCheck]:
        ...
        for child_zone, parent_zone, parent_zone_ns in chain_info:
            ...
            if parent_zone_ns:
                ns_records_in_parent = self.get_ns_records_for_domain(
                    child_zone, parent_zone_ns
                )
                ...
                if not self.is_legitimate_second_level_domain(child_zone):
                    problematic_records = self.check_problematic_records(
                        child_zone, parent_zone_ns
                    )
            ...
            if not parent_zone_ns:
                ...
                record.startswith("SOA:") or record.startswith("SOA ")
                for record in problematic_records
            ):
                status = DelegationStatus.EXTRA_SOA
            elif any(
                record.startswith("A:") or record.startswith("A ")
                for record in problematic_records
            ):
                status = DelegationStatus.EXTRA_A
            elif any(
                record.startswith("AAAA:") or record.startswith("AAAA ")
                for record in problematic_records
            ):
                status = DelegationStatus.EXTRA_AAAA
            elif any(
                record.startswith("CNAME:") or record.startswith("CNAME ")
                for record in problematic_records
            ):
                status = DelegationStatus.EXTRA_CNAME
            elif any(
                record.startswith("MX:") or record.startswith("MX ")
                for record in problematic_records
            ):
                status = DelegationStatus.EXTRA_MX
            elif any(
                record.startswith("TXT:") or record.startswith("TXT ")
                for record in problematic_records
            ):
                status = DelegationStatus.EXTRA_TXT
            elif problematic_records:
                status = DelegationStatus.OTHER_RECORDS
            elif ns_records_in_child and set(ns_records_in_parent) != set(
                ns_records_in_child
            ):
    ...
    def print_results(
        self, target_domain: str, results: List[DelegationCheck], explain: bool = False
    ):
        ...
        for i, check in enumerate(results):
            ...
            if check.ns_records_in_parent:
                print(
                    f"  NS Records in Parent Zone ({check.parent_zone}): {', '.join(check.ns_records_in_parent)}"
                )

            if check.ns_records_in_child:
                print(
                    f"  NS Records in Child Zone ({check.zone_name}): {', '.join(check.ns_records_in_child)}"
                )
            ...
            if explain and check.status != DelegationStatus.PROPER:
                ...
                if check.status == DelegationStatus.ERROR:
                    print(
                        f"    ❌ ERROR: Could not find authoritative nameservers for the parent zone '{check.parent_zone}'. This may indicate that the domain is not registered or there are DNS resolution issues."
                    )
                elif check.status == DelegationStatus.MISSING_NS:
                    print(
                        f"    ❌ MISSING_NS: No NS records found for '{check.zone_name}' in the parent zone '{check.parent_zone}'. This means the subdomain is not delegated and may not exist as a separate zone."
                    )
                elif check.status == DelegationStatus.EXTRA_SOA:
                    print(
                        f"    ❌ EXTRA_SOA: Found SOA record for '{check.zone_name}' in parent zone '{check.parent_zone}'. SOA records define zone authority - the parent zone should not have SOA records for child zones. This suggests the parent is trying to be authoritative for the child zone, which conflicts with proper delegation."
                    )
                elif check.status == DelegationStatus.EXTRA_A:
                    print(
                        f"    ❌ EXTRA_A: Found A record for '{check.zone_name}' in parent zone '{check.parent_zone}'. A records map hostnames to IPv4 addresses. In proper delegation, the parent zone should only contain NS records pointing to the child zone's nameservers. A records suggest the parent is handling the zone directly."
                    )
                elif check.status == DelegationStatus.EXTRA_AAAA:
                    print(
                        f"    ❌ EXTRA_AAAA: Found AAAA record for '{check.zone_name}' in parent zone '{check.parent_zone}'. AAAA records map hostnames to IPv6 addresses. Same issue as A records - the parent zone should not handle the child zone directly."
                    )
                elif check.status == DelegationStatus.EXTRA_CNAME:
                    print(
                        f"    ❌ EXTRA_CNAME: Found CNAME record for '{check.zone_name}' in parent zone '{check.parent_zone}'. CNAME records create aliases for hostnames. CNAME records should not exist for zones that are being delegated, as they can cause DNS resolution conflicts."
                    )
                elif check.status == DelegationStatus.EXTRA_MX:
                    print(
                        f"    ❌ EXTRA_MX: Found MX record for '{check.zone_name}' in parent zone '{check.parent_zone}'. MX records specify mail servers for a domain. If the parent zone has MX records for the child zone, it suggests the parent is trying to handle mail for the child zone, which conflicts with delegation."
                    )
                elif check.status == DelegationStatus.EXTRA_TXT:
                    print(
                        f"    ❌ EXTRA_TXT: Found TXT record for '{check.zone_name}' in parent zone '{check.parent_zone}'. TXT records contain text information. While not always problematic, TXT records in the parent zone for the child zone can indicate the parent is trying to handle the child zone directly."
                    )
                elif check.status == DelegationStatus.OTHER_RECORDS:
                    print(
                        f"    ❌ OTHER_RECORDS: Found other record types for '{check.zone_name}' in parent zone '{check.parent_zone}'. Any record types in the parent zone for the child zone (except NS records) indicate improper configuration and can cause DNS resolution issues."
                    )
                elif check.status == DelegationStatus.NS_MISMATCH:
                    print(
                        f"    ❌ NS_MISMATCH: The NS records in the parent zone '{check.parent_zone}' for '{check.zone_name}' do not match the actual nameservers of the child zone. This indicates a configuration mismatch that can cause DNS resolution failures."
                    )
                print(
                    f"    💡 Solution: In proper delegation, the parent zone should only contain NS records pointing to the child zone's authoritative nameservers. Remove all other record types for the child zone from the parent zone."
                )
...
def parse_arguments():
    ...
    parser.add_argument(
        "domain", help="Target domain to test (e.g., test.mail.google.com)"
    )
    parser.add_argument("--export", help="Export results to JSON file")
    parser.add_argument(
        "--explain",
        action="store_true",
        help="Provide detailed explanations for each error found",
    )
    parser.add_argument(
        "--no-dynamic-roots",
        action="store_true",
        help="Use hardcoded root servers instead of dynamic fetching",
    )
