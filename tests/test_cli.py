...
class TestCLI:
    ...
    def test_basic_usage(self, mock_checker_class):
        ...
        with patch.object(mock_checker, "print_results") as mock_print:
            ...
            mock_print.assert_called_once_with(
                "example.com", mock_results, explain=False
            )
    ...
    def test_usage_with_explain(self, mock_checker_class):
        ...
        with patch.object(mock_checker, "print_results") as mock_print:
            ...
            mock_print.assert_called_once_with(
                "example.com", mock_results, explain=True
            )

    @patch(
        "sys.argv",
        ["dns_delegation_checker.py", "example.com", "--export", "test.json"],
    )
    @patch("dns_delegation_checker.DNSDelegationChecker")
    ...
    @patch(
        "sys.argv", ["dns_delegation_checker.py", "example.com", "--no-dynamic-roots"]
    )
    @patch("dns_delegation_checker.DNSDelegationChecker")
    ...
    def test_print_results_with_explanations(self):
        ...
        with patch("sys.stdout", new=StringIO()) as mock_stdout:
            ...
            assert any(
                ...
                for word in ["authority", "delegation", "problematic"]
            )
