...
from dns_delegation_checker import DNSDelegationChecker, DelegationStatus
...
class TestEdgeCases:
    ...
    def test_legitimate_second_level_edge_cases(self, checker):
        ...
        for domain in legitimate_domains:
            assert checker.is_legitimate_second_level_domain(domain), f"Failed for {domain}"
        ...
        for domain in non_legitimate_domains:
            assert not checker.is_legitimate_second_level_domain(domain), f"Failed for {domain}"
    ...
    def test_delegation_chain_with_errors(self, checker):
        ...
        with patch.object(checker, 'get_nameservers_for_zone') as mock_nameservers:
            ...
            with patch.object(checker, 'get_ns_records_for_domain') as mock_ns:
                ...
                with patch.object(checker, 'check_problematic_records') as mock_problematic:
